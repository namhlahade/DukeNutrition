...
def todaysInfo():
    ...
    query = db.execute("select name, calories, protein, carbs, fat from User_Meal where user_id = ? and meal_date = ?", (userid, date.today().isoformat(),))
...
def thisWeeksStats():
    ...
    for date in meal_date:
        query = db.execute("select * from User_Meal where meal_date = ? and user_id = ?", (date, userid,))
